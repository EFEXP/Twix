...
import xyz.donot.quetzal.util.extrautils.startForResult
...
class UserDetailFragment() : RxFragment()
{
  ...
  val userId: Long by lazy { arguments.getLong("userId", 0L) }
  ...
  fun setUpView(user: User) {
    if (user.id != getMyId()) {
      ...
      edit_profile.setOnClickListener {
        activity.startForResult<EditProfileActivity>(1)
      }
    }
    ...
    if (user.isProtected) {
      user_name.setCompoundDrawablesWithIntrinsicBounds(null, null, re, null)
    }
